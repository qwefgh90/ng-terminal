...
describe('CsiFunction', () => {
  ...
  it('eraseInDisplay()', () => {
    ...
    expect(csi.eraseInDisplay(KindOfEraseInDisplay.SavedLines)).toBeTruthy(
      '\x9b3J',
    );
  });
  it('eraseSelectiveThingsInDisplay()', () => {
    expect(
      csi.eraseSelectiveThingsInDisplay(KindOfEraseInDisplay.Above),
    ).toBeTruthy('\x9b?1J');
    expect(
      csi.eraseSelectiveThingsInDisplay(KindOfEraseInDisplay.All),
    ).toBeTruthy('\x9b?2J');
    expect(
      csi.eraseSelectiveThingsInDisplay(KindOfEraseInDisplay.Below),
    ).toBeTruthy('\x9b?0J');
    expect(
      csi.eraseSelectiveThingsInDisplay(KindOfEraseInDisplay.SavedLines),
    ).toBeTruthy('\x9b?3J');
  });
  it('eraseInLine()', () => {
  ...
  it('eraseSelectiveThingsInLine()', () => {
    expect(csi.eraseSelectiveThingsInLine(KindOfEraseInLine.Right)).toBeTruthy(
      '\x9b?0K',
    );
    expect(csi.eraseSelectiveThingsInLine(KindOfEraseInLine.Left)).toBeTruthy(
      '\x9b?1K',
    );
    expect(csi.eraseSelectiveThingsInLine(KindOfEraseInLine.All)).toBeTruthy(
      '\x9b?2K',
    );
  });
  it('insertLines()', () => {
